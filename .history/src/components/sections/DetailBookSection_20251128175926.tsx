...
export default function DetailBookSection({ bookId }: DetailBookSectionProps) {
  ...
  useEffect(() => {
    if (bookId) {
      dispatch(fetchBookById(bookId.toString()));
    }
  }, [bookId, dispatch]);useEffect(() => {
    dispatch(fetchBookById(bookId.toString()));
  }, [bookId, dispatch]);
  ...
  const handleAddToCart = async () => {
    ...
    try {
      await axios.post("https://be-library-api-xh3x6c5iiq-et.a.run.app/api/cart", {
        bookId: book.id,
        quantity: 1,
      });
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2 text-sm text-gray-500 mb-4">
        ...
        <span>/</span>
        <Link to={`/category/${book.Category?.id ?? ""}`} className="hover:text-black hover:underline">
          Category
        </Link>
        <span>/</span>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="col-span-2">
          ...
          <div className="flex items-center gap-4">
            <Button variant="outline" className="px-6 py-3 rounded-full" onClick={handleAddToCart} disabled={adding}>

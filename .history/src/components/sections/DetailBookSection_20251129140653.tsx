...
import { incrementCartCount } from "@/features/cart/cartSlice"; // pastikan ada action ini
...
export default function DetailBookSection({ bookId }: DetailBookSectionProps) {
  ...
  const handleAddToCart = async () => {
    ...
    // ===== Optimistic UI =====
    dispatch(incrementCartCount()); // langsung update jumlah cart
    ...
    try {
      ...
      // Jika ingin, bisa fetch ulang stok:
      setBook(prev => prev ? { ...prev, stock: (prev.stock ?? 1) - 1 } : prev);
    } catch (err: unknown) {
      // rollback jika error
      dispatch({ type: "cart/decrement" }); // rollback cart count jika ada error
      if (axios.isAxiosError(err)) {
        const axiosErr = err as AxiosError;
        const errorData = axiosErr.response?.data as { message?: string };
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="col-span-2">
          ...
          <div className="flex items-center gap-4">
            ...
            >
              {adding ? "Adding..." : (book.stock ?? 0) <= 0 ? "Out of Stock" : "Add to Cart"}
            </Button>

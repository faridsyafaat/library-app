 ...
 <div className="w-full container py-6">
  <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
    ...
    {categories.map((cat) => {
      ...
      // DEBUG: cek isi image dari API
      console.log("CAT IMAGE VALUE:", cat.image);

      return (
        <div key={cat.id} className="flex flex-col items-center">
         <img
  src={cat.image ? `https://be-library-api-xh3x6c5iiq-et.a.run.app/${cat.image}` : "/image/fallback.png"}
  alt={cat.name}
  className="w-[162px] h-[64px] object-contain mb-2"
  onError={(e) => {
    if (e.currentTarget.src.includes("fallback.png")) return; // cegah loop

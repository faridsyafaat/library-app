...
import { addToCartAsync } from "@/store/cartSlice"; // sesuaikan path jika berbeda
...
export default function DetailBookSection({ id }: DetailBookSectionProps) {
  const dispatch = useAppDispatch();
  const navigate = useNavigate();

  // ambil detail book dari store (bookDetailSlice)
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 text-sm text-gray-500 mb-4">
        <Link to="/homeafter" className="hover:text-black hover:underline">Home</Link>
        <span>/</span>
        <Link to={`/category/${book.Category?.id ?? ""}`} className="hover:text-black hover:underline">Category</Link>
        <span>/</span>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="col-span-1 flex justify-center md:justify-start">
          <img src={book.coverImage} alt={book.title} className="w-2/3 md:w-full h-auto object-cover rounded-xl shadow" />
        </div>
        ...
        <div className="col-span-2">
          ...
          <p className="text-lg text-gray-600 mb-3 cursor-pointer hover:underline" onClick={() => navigate(`/author/${book.Author?.id}`)}>
          ...
          <div className="flex items-center gap-4">
            <Button variant="outline" className="px-6 py-3 rounded-full" onClick={handleAddToCart}>
              Add to Cart
            </Button>

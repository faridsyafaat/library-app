...
import { addToCartAsync } from "@/store/cartSlice"; // sesuaikan path kalau perlu
...
export default function DetailBookSection() {
  ...
  // --- SATU handler saja untuk Add to Cart ---
  const handleAddToCart = () => {
    if (!book) return;

    // Pastikan tipe id sesuai API (convert ke number bila API mengharapkan number)
    const bookId = typeof book.id === "string" ? Number(book.id) : book.id;

    // dispatch async thunk
    dispatch(addToCartAsync({ bookId }));

    // optional: langsung redirect ke mycart
    // navigate("/mycart");
  };

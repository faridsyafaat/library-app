...
/**
 * Minimal type untuk BookDetail yang dipakai komponen ini.
 * Jika kamu sudah punya `BookDetail` type di project, ganti import dan hapus type ini.
 */
interface Author {
  id?: number;
  name?: string;
...
interface BookDetail {
  ...
  // Tambah field lain kalau backend mengembalikan lebih banyak
}
...
export default function DetailBookSection({ bookId }: DetailBookSectionProps) {
  ...
  useEffect(() => {
    const fetchBook = async () => {
      ...
      try {
        ...
        // Normalisasi response: backend mungkin mengembalikan { data: {...} } atau langsung object
        const payload = res.data?.data ?? res.data;
        setBook(payload as BookDetail);
      } catch (err: unknown) {
        // Tangani error secara aman tanpa 'any'
        if (axios.isAxiosError(err)) {
          const axiosErr = err as AxiosError;
          setError(axiosErr.response?.data?.message ?? "Failed to load book");
        } else {
  ...
  const handleAddToCart = async () => {
    ...
    try {
      ...
      if (axios.isAxiosError(err)) {
        const axiosErr = err as AxiosError;
        setAddError(axiosErr.response?.data?.message ?? "Failed to add to cart");
      } else {
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="p-6 max-w-6xl mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="col-span-1 flex justify-center">
          <img src={book.coverImage} alt={book.title} className="w-2/3 md:w-full rounded-xl shadow" />
        </div>

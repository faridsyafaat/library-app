      ...
      <div className="container mx-auto px-6">
        ...
        <div className="mt-12 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-6">
          {bookCategories.map((book) => (
            <div
              key={book.name}
              className="flex flex-col items-center text-left cursor-pointer hover:scale-105 transition-transform"
            >
              <img
                src={book.img}
                alt={book.name}
                className="width-[162.67px] height-[64px] object-cover rounded-lg mb-2"
              />

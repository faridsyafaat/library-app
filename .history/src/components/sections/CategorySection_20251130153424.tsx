...
// ===== mapping gambar (key = normalized name) =====
...
// normalizer -> lowercase, hapus spasi & strip
...
export default function CategorySection() {
  ...
  // buat dictionary utk lookup cepat berdasarkan exact name dari API
  const apiDict = categories.reduce((acc, cat) => {
    acc[cat.name] = cat;
    return acc;
  }, {} as Record<string, Category>);

  // finalCategories selalu 6 item sesuai targetOrder
  ...
  return (
    <div className="w-full container py-6">
      {/* mobile: 3 kolom (jadi 2 baris untuk 6 item), md+ : 6 kolom (1 baris) */}

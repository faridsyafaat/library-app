...
async function borrowBook(id: string) {
  const res = await fetch(
    `https://be-library-api-xh3x6c5iiq-et.a.run.app/api/books/1/${id}/borrow`,
    {
      method: "POST",
...
async function addToCartApi(book: { id: number }) {
  const res = await fetch("https://be-library-api-xh3x6c5iiq-et.a.run.app/api/books/recommend?by=rating&limit=10", {
    method: "GET",
    headers: { "Content-Type": "application/json" },
...
export default function DetailBookSection() {
  ...
  if (!data) return <div className="p-4">Book not found</div>;

...
import { addToCartAsync } from '@/store/cartSlice'; // <- pastikan path ini benar (lihat catatan di bawah)
...
export default function DetailBookSection() {
  ...
  // Handler yang digunakan untuk dispatch addToCartAsync
  const handleAddToCart = () => {
    if (!book) return;

    // Pastikan tipe id sesuai dengan API (number/string)
    const bookId = typeof book.id === 'string' ? Number(book.id) : book.id;

    dispatch(addToCartAsync({ bookId }));
    // optional: langsung redirect ke halaman cart
    // navigate('/mycart');
  };
        ...
        <div className='col-span-2'>
          ...
          <div className='flex items-center gap-4'>
            <Button
              ...
              onClick={handleAddToCart} // <- benar: onClick
            >
              Add to Cart
            </Button>

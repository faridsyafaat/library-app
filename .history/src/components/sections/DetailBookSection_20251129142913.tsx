...
export default function DetailBookSection({ bookId }: DetailBookSectionProps) {
  ...
  const handleAddToCart = async () => {
    ...
    try {
      ...
      // Redirect ke My Cart setelah 0.8 detik supaya toast terlihat
    ...
    >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="col-span-2">
          ...
          <div className="flex items-center gap-4">
            <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
              <Button
                ...
                disabled={adding || isOutOfStock} // tombol disable saat stok habis
              >
                {addToCartLabel}
              </Button>
            </motion.div>
          ...
          {isOutOfStock && <p className="text-red-500 mt-2">Out of Stock</p>}
        </div>
      </div>
    </motion.div>
  );

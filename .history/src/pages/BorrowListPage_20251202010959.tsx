...
type ComputedStatus = "Active" | "Returned" | "Overdue";
type LoanWithStatus = LoanItem & { computedStatus: ComputedStatus };
...
export default function BorrowListPage() {
  ...
  const loansWithStatus: LoanWithStatus[] = useMemo(() => {
    const today = new Date();
    return loans.map((ln) => {
      const due = new Date(ln.dueAt);
      let computed: ComputedStatus = "Active";
      if (ln.returnedAt) computed = "Returned";
      else if (due < today) computed = "Overdue";
      return { ...ln, computedStatus: computed };
    });
  }, [loans]);

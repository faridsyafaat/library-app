...
export default function CategoryPage() {
  ...
  return (
    <>
     ...
     <h1 className="text-2xl font-bold fixed top-16 left-0 right-0 bg-white z-30 py-4 px-4 container">
        Book List
      </h1>

      <div className="flex w-full gap-x-4 mt-[6rem] container mx-auto">
  {/* Sidebar kiri */}
  <div className="w-1/4 sticky top-[6rem] max-h-[calc(100vh-6rem)] overflow-y-auto z-20">
          <FilterSidebar />
        </div>

        {/* Konten kanan */}
        <div className="w-3/4 min-h-[calc(100vh-6rem)]">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-x-6 gap-y-8 mt-4 justify-start h-[439px] w-[204.75px]">
  {books.map((book) => (
    <Link
      key={book.id}
      to={`/books/${book.id}`}
     className="bg-white rounded-xl shadow-sm gap-6 p-4 hover:shadow-md cursor-pointer flex flex-col w-[204.75px] h-[439.12px]"
    >
      <img
        src={book.coverImage}
        alt={book.title}

...
export default function MyCartPage() {
  ...
  const token = useAppSelector((state) => state.auth.token); // <---- sudah typed dan aman
  ...
  // MUTATION — BORROW BOOK
  const borrowMutation = useMutation<CartResponse, Error, { cartId: number; items: number[] }>({
  ...
  return (
    <>
    <NavbarAfter />
   <div className="container grid grid-cols-1 md:grid-cols-3 gap-6 p-10">
      {/* CART ITEMS */}
      <div className="md:col-span-2 space-y-4">
        <h1 className="text-2xl font-semibold mb-4">My Cart</h1>

        <div className="w-[642px] h-[138px]flex items-center gap-2 mb-4">
          <Checkbox
            checked={allSelected}
        ...
        <div className="space-y-4">
          {items.map((item) => (
            <Card key={item.id} className="p-4 flex gap-4 items-center w-[318px] h-[200px]">
              <Checkbox
          ...
          {items.length === 0 && (
            <div className="w-[316px]h-[200px] text-center text-muted-foreground p-6">
              Keranjang kosong.
            </div>
          )}
        </div>

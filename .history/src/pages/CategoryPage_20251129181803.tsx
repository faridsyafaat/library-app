...
type Category = {
  id: number;
  name: string; // <-- perbaikan: pakai string kecil
};

export default function CategoryPage() {
  const { id } = useParams<{ id: string }>();
  const [books, setBooks] = useState<Book[]>([]);
  const [categoryName, setCategoryName] = useState<string>(""); // <-- string kecil
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        // Filter buku sesuai categoryId
        const filteredBooks = allBooks.filter(
          (b) => b.categoryId === Number(id)
        );
        setBooks(filteredBooks);

        // Ambil nama category dari endpoint categories
        ...
        setCategoryName(cat ? cat.name : ""); // <-- hilangkan fallback String object
      } catch (error) {
    ...
    fetchData();
  }, [id]); // setter dari useState tidak perlu ditambahkan di deps

...
export default function BorrowListPage() {
  ...
  useEffect(() => {
    ...
    async function fetchLoans() {
      ...
      try {
        ...
        // assume body.data.loans is the array
  ...
  // Return book (calls API PATCH /api/loans/:id/return or similar). We'll try PATCH /api/loans/:id/return
  const handleReturn = async (loanId: number) => {
    ...
    try {
      ...
      console.error(err);
      alert((err as Error).message || "Return failed");
    }
  };
    ...
    <>
      ...
      <div className="container mx-auto py-10">
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              <button
                className={`px-3 py-1 rounded-full ${filter === "all" ? "bg-gray-200" : "bg-white border" }`}
                onClick={() => setFilter("all")}
              >
                All
              </button>
              <button
                className={`px-3 py-1 rounded-full ${filter === "active" ? "bg-green-100" : "bg-white border" }`}
                onClick={() => setFilter("active")}
              >
                Active
              </button>
              <button
                className={`px-3 py-1 rounded-full ${filter === "returned" ? "bg-gray-100" : "bg-white border" }`}
                onClick={() => setFilter("returned")}
              >
                Returned
              </button>
              <button
                className={`px-3 py-1 rounded-full ${filter === "overdue" ? "bg-red-100" : "bg-white border" }`}
        ...
        {loading ? (
          ...
          <div className="py-20 text-center text-gray-600">No borrowed books found.</div>
        ) : (
          <div className="space-y-4">
            {visible.map((ln) => (
              <div key={ln.id} className="flex items-center bg-white rounded-lg shadow-sm p-4">
                <img
                ...
                <div className="flex-1 px-4">
                  <div className="flex items-start justify-between gap-4">
                    <div>
                      <div className="text-xs text-gray-500 mb-1">Status
                        <span className={`ml-2 px-2 py-0.5 rounded-full text-xs ${ln.computedStatus === 'Active' ? 'bg-green-100 text-green-700' : ln.computedStatus === 'Returned' ? 'bg-gray-100 text-gray-700' : 'bg-red-100 text-red-700'}`}>
                          {ln.computedStatus}
                        </span>
                      </div>
                    ...
                    <div className="text-right">
                      ...
                      <div className="mt-4 flex flex-col gap-2">
                        <button className="px-4 py-2 bg-blue-600 text-white rounded" onClick={() => navigate(`/borrow-detail/${ln.id}`)}>
                          View Details
                        </button>

                        {/* Give Review - dummy navigation */}
                        <button className="px-4 py-2 border rounded" onClick={() => navigate(`/reviews/create?bookId=${ln.bookId}`)}>
                          Give Review
                        </button>

                        {/* Return button only when active or overdue */}
                        {(ln.computedStatus === "Active" || ln.computedStatus === "Overdue") && (
                          <button className="px-4 py-2 bg-gray-100 rounded text-sm" onClick={() => handleReturn(ln.id)}>
                            Return Book
                          </button>
                        )}
                      </div>

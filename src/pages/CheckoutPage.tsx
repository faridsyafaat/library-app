...
const CheckoutPage = () => {
  ...
  const { selectedItems } =
    (location.state as { selectedItems: SelectedBook[] }) || {
      selectedItems: [],
    };
  ...
  const returnDate = new Date(
    borrowDate.getTime() + duration * 24 * 60 * 60 * 1000
  );
  ...
  if (!selectedItems || selectedItems.length === 0) {
    return (
      <div className="p-6">No books selected. Please go back to cart.</div>
    );
  }
  ...
  const handleBorrow = async () => {
    ...
    try {
      let lastDueAt: string | null = null; // ⭐ MENYIMPAN dueAt dari API
      ...
      for (const item of selectedItems) {
        ...
        lastDueAt = data.data?.dueAt; // ⭐ simpan tanggal dari server
      }

      // ⭐ FORMAT TANGGAL DARI API
      const formattedReturnDate = lastDueAt
        ? new Date(lastDueAt).toLocaleDateString("en-US", {
            day: "2-digit",
            month: "long",
            year: "numeric",
          })
      ...
      // ⭐ REDIRECT KE BorrowSuccessPage + kirim tanggal
  ...
  return (
    <>
      <NavbarAfter />
      <h1 className="container text-4xl font-bold mb-6 mt-8">Checkout</h1>
      <h2 className="container text-2xl font-bold mb-4">User Information</h2>

      <div className="container mx-auto grid grid-cols-1 md:grid-cols-2 gap-6">
        {/* LEFT */}
        <div>
          <section className="flex justify-between h-[138px]">
          ...
          <section>
            ...
            {selectedItems.map((item: SelectedBook) => (
              <div
                key={item.id}
                className="flex gap-4 items-start border p-3 rounded-lg"
              >
                <img
